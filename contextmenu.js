...
const ContextMenu = (() => {
  ...
  const SettingsModal = (() => {
    ...
    const searchBarPresetBtns = document.querySelectorAll(
      ".search-bar-preset-btn",
    );
    ...
    const iconImageSizeInput = document.getElementById(
      "settings-icon-image-size",
    );
    ...
    iconPresetBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        Theme.applyIconPreset(
          preset,
          parseInt(iconRadiusInput.value, 10),
          parseInt(iconImageSizeInput.value, 10),
        );
    ...
    colsInput.addEventListener("change", () => {
      const cols = Math.min(
        10,
        Math.max(3, parseInt(colsInput.value, 10) || 5),
      );
    ...
    function open() {
      ...
      iconPresetBtns.forEach((btn) => {
        btn.classList.toggle(
          "active",
          btn.dataset.preset === currentIconPreset,
        );
      });
      customIconSettings.classList.toggle(
        "hidden",
        currentIconPreset !== "custom",
      );
      ...
      searchBarPresetBtns.forEach((btn) => {
        btn.classList.toggle(
          "active",
          btn.dataset.preset === currentSearchBarPreset,
        );
      });
